...
export const BoxesCore = ({ className, ...rest }: { className?: string }) => {
  ...
  return (
    ...
    >
      {rows.map((_, i) => (
        <motion.div key={`row` + i} className='relative h-16 w-32 border-l border-stone-700'>
          {cols.map((_, j) => (
            <motion.div
              whileHover={{
              ...
              key={`col` + j}
              className='relative h-16 w-32 border-r border-t border-stone-700'
            >
              {j % 2 === 0 && i % 2 === 0 ? (
                <svg
                  ...
                  strokeWidth='1.5'
                  stroke='currentColor'
                  className='pointer-events-none absolute -left-[22px] -top-[14px] h-6 w-10 stroke-[1px] text-stone-700'
                >
